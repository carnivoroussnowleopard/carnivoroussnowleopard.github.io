...
async function loadGameDetail() {
  ...
  // 각 API 호출을 개별 try-catch로 분리해서 어디서 실패하는지 파악
  let game, shots, stores;

  try {
    game = await fetchGame(gameId);
  } catch (e) {
    console.error('[GamePick] fetchGame 실패:', e);
    const msg = e.message.includes('401') ? 'API 키가 유효하지 않습니다.'
              : e.message.includes('404') ? `게임(ID: ${gameId})을 찾을 수 없습니다.`
              : `네트워크 오류: ${e.message}`;
    container.innerHTML = `
      <div class="detail-loading">
        <p style="color:var(--text2)">${msg}</p>
        <p style="margin-top:12px">
          <a href="index.html" style="color:var(--accent2)">← 홈으로 돌아가기</a>
        </p>
      </div>`;
    return;
  }
  ...
  // 스크린샷·스토어는 실패해도 무시
  [shots, stores] = await Promise.all([
    fetchScreenshots(gameId).catch(() => ({ results: [] })),
    fetchStores(gameId).catch(() => ({ results: [] })),
  ]);
  ...
  const platformBadges = (game.platforms || [])
    ...
    .map(p => {
      ...
      if (!name) return '';
      return `<span class="badge badge-platform">${shortPlatform(name)}</span>`;
    })
    .join('');
  ...
  const desc = rawDesc
    ? rawDesc.slice(0, 800) + (rawDesc.length > 800 ? '...' : '')
    : '게임 설명이 없습니다.';

  // ─── 태그 ────────────────────────────────────────────
  const tags = (game.tags || [])
    .slice(0, 8)
    .map(t => `<span class="genre-tag">${escHtml(t.name)}</span>`)
    .join('');
  ...
  const firstGenreSlug = (game.genres || [])[0]?.slug || '';

  container.innerHTML = `
    <div class="detail-hero">
      ${heroImg ? `<img class="detail-hero-img" src="${escHtml(heroImg)}" alt="${escHtml(game.name)}" />` : ''}
      <div class="detail-hero-overlay"></div>
      <div class="detail-hero-content">
        <h1 class="detail-title">${escHtml(game.name)}</h1>
        <div class="detail-badges">
          ${rating !== '—' ? `<span class="badge badge-rating">⭐ ${rating}</span>` : ''}
          <span class="badge badge-year">${escHtml(year)}</span>
          ${platformBadges}
        </div>
      </div>
    </div>
      ...
      <div class="detail-sidebar">
        <div class="detail-info-card">
          ...
          <div class="detail-info-row">
            ...
            <span class="info-value">${escHtml(game.released || '미정')}</span>
          </div>
          <div class="detail-info-row">
            <span class="info-label">장르</span>
            <span class="info-value">${escHtml(genresStr)}</span>
          </div>
          <div class="detail-info-row">
            <span class="info-label">개발사</span>
            <span class="info-value">${escHtml(devs)}</span>
          </div>
          <div class="detail-info-row">
            <span class="info-label">퍼블리셔</span>
            <span class="info-value">${escHtml(pubs)}</span>
          </div>
          ${game.playtime ? `
          <div class="detail-info-row">
        ...
        <a href="search.html?genre=${encodeURIComponent(firstGenreSlug)}"
